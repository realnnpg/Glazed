...
public class SwordAxePlaceCobweb extends Module {
    private final MinecraftClient mc = MinecraftClient.getInstance();
    private boolean placing = false;
    private int previousSlot = -1;

    public SwordAxePlaceCobweb() {
        super(GlazedAddon.pvp, "sword-axe-place-cobweb", "Right-click with sword or axe to place cobweb, then switch back.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (!isSwordOrAxe(mainHand)) return;

        if (mc.options.useKey.isPressed() && !placing) {
            int cobwebSlot = findCobwebSlot();
            ...
            mc.player.networkHandler.sendPacket(new UpdateSelectedSlotC2SPacket(cobwebSlot));
